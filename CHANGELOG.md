...
- Sprint 2 (Chat), step 1: the AI layer now runs on the server. Added `app/api/chat/route.ts`, which validates the conversation, builds the system prompt with real dashboard context (profile, finances, goals, agenda, memories), executes `AIService` server-side and streams SSE back. The browser no longer constructs `AIService` and never sees the API key. `ChatView` consumes the route through `lib/chat/client.ts`. The conversational surface moved to `/` where the sidebar always pointed, and the orphaned `/chat` route was removed. `/` dropped from 227 kB to 202 kB first-load JS. Verified: request validation, missing-key handling, provider-error mapping and end-to-end streaming against a mock NIM endpoint.
- Repository cleanup: removed dead modules and duplicate rendering in the chat, deleted 20 README-only scaffold directories and 40 specifications for systems that were never built, and replaced the README-presence CI check with typecheck, lint and build gates.
- Sprint 1 complete: Dashboard (Mission Control) production-ready. Added Agenda section with event/payment/reminder/meeting/commitment kinds, connected to typed data contracts and demo data. Visual polish: consistent spacing, hover/focus states, reduced-motion support. Accessibility review: ARIA labels, semantic HTML, color contrast. Performance: lazy-loaded below-fold sections, skeleton loading, optimized bundle.
...
- Added the AI Provider Layer implementation under `services/ai/` including routing, registry, adapters, and discovery.
